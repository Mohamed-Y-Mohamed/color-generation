...
export function WCAGComplianceIndicator({ 
  ...
  return (
    <Badge variant={getVariant()} className="text-xs flex items-center space-x-1">
      {getIcon()}
      <span>{getLabel()}</span>
    </Badge>
  );
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex justify-between text-xs">
        <span className={contrastRatio >= 4.5 ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}>
          AA (4.5:1)
        </span>
        <span className={contrastRatio >= 7 ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}>
          AAA (7:1)
        </span>
      </div>
    </div>
